...
describe 'etckeeper::config' do
  ...
  context 'with attribute use_remote' do
    ...
    # it 'adds a ssh config for root' do
    #   expect(chef_run).to add_ssh_config('root-etckeeper')
    #     .with(user: 'root')
    #     .with(host: chef_run.node['etckeeper']['git_host'])
    # end

    it 'creates a etckeeper_git_remote resource' do
      attr = chef_run.node['etckeeper']
      expect(chef_run).to create_etckeeper_git_remote(//)
        .with(host: attr['git_host'])
        .with(repository: attr['git_repo'])
    ...
    context 'without set git remote' do
      ...
      before do
        ...
        remote = 'ssh://git@github.com/etckeeper'

        # stub setting remote
        allow(Mixlib::ShellOut).to receive(:new)
          .with("#{git_cfg} --add remote.origin.url #{remote}")
        ...
        allow(Mixlib::ShellOut).to receive(:new)
          .with("#{git_cfg} --get remote.origin.url #{remote}")
      ...
      it 'creates the ssh config for root' do
        expect(chef_run).to add_ssh_config(
          chef_run.node['etckeeper']['git_host']
        )
      end

      it 'creates the ssh identity file' do
        expect(chef_run).to create_file(
          "/root/.ssh/id_#{chef_run.node['etckeeper']['git_host']}"
        )
      end
    end
